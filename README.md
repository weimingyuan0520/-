...
| 模式 | Server |
| 端口 | 8080 |
| 数据格式 | JSON (与串口协议相同，兼容带空格/无空格两种格式) |
| 换行符 | `\r\n` |

> 💡 JSON 指令兼容 `{"cmd":"set_threshold",...}` 和 `{"cmd": "set_threshold", ...}` 两种格式（带空格/不带空格均可）。
...
| **K1** | 长按 2s | 快速跳转: 非P1→跳P1阈值页 / P1→跳P0 |
| **K2** | 短按 (P1阈值页) | 当前阈值 -50 |
| **K2** | 长按 (P1阈值页) | 当前阈值 -200 |
| **K2** | 短按 (P0/P2页面) | 强制关闭蜂鸣器 / 恢复自动 |
| **K3** | 短按 (P1阈值页) | 当前阈值 +50 |
| **K3** | 长按 (P1阈值页) | 当前阈值 +200 |
| **K3** | 短按 (P0/P2页面) | 蜂鸣器测试开关 (强开↔自动) |
| **K3** | 长按 (P0/P2页面) | 蜂鸣器测试开关 (强开↔自动) |

> 💡 调整阈值只需短按 K1 切到 **P1 Thresholds 页面**，用 K2/K3 直接调节。切回 P0 后新阈值立即生效，无需进入"配置模式"。
...
> 💡 切到 P1 页面后直接用 K2/K3 调整阈值，无需进入"配置模式"。
